...
import org.grayhack.util.world.DamageUtils;
	...
	public void onTick(EventTick event) {
		...
		if (smartDisabled && logText == null) {
			...
			if (getSetting(4).asToggle().getChild(0).asToggle().getState()) {
				GrayLogger.info("Re-enabled AutoLog!");
			}
	...
	private Text getLogText() {
		...
		if (getSetting(1).asToggle().getState() && !hasTotem) {
			for (PlayerEntity player: mc.world.getPlayers()) {
				if ((!getSetting(1).asToggle().getChild(0).asToggle().getState() && GrayHack.friendMang.has(player))
						|| player == mc.player) {
					continue;
				}
		...
		if (getSetting(3).asToggle().getState()) {
			...
			for (PlayerEntity player: mc.world.getPlayers()) {
				if (!EntityUtils.isOtherServerPlayer(player)
						|| (!getSetting(3).asToggle().getChild(1).asToggle().getState() && GrayHack.friendMang.has(player))) {
					continue;
				}
